    ...
    let heartBeatInterval: NodeJS.Timer | null = setInterval(async () => {
      if (heartBeatStatistics.current && heartBeatStatistics.current.vm_ip) {
        ...
        heartBeatStatistics.current = {
          ...
          ram: {
            //@ts-ignore
            total: 1024, //parseInt(tizen.systeminfo.getTotalMemory() / (1024 * 1024)),
            //@ts-ignore
            available: 100, //parseInt(tizen.systeminfo.getAvailableMemory() / (1024 * 1024)),
          },
          stats: {
            ...heartBeatStatistics.current,
            decoder: "test decoder", //statsResponse.decoder,
            total_received_fps: 60, //parseInt(statsResponse.received_fps),
            total_render_fps: 60, //parseInt(statsResponse.rendered_fps),
            total_decode_time: 0.2, //parseInt(statsResponse.decode_time),
            received_fps: 60,
            ...
            net_latency: 0, //parseFloat(statsResponse.net_latency),
            variance: 0, //parseFloat(statsResponse.variance),
          },
        };
