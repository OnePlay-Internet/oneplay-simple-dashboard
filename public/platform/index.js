...
function stopBackgroundPollingOfHost(host) {
  console.log(
    "%c[index.js, backgroundPolling]",
    "color: green;",
    "Stopping background polling of host " + host.serverUid + "\n",
    host,
    host.toString()
...
function pairTo(nvhttpHost, hostSessionKey, onSuccess, onFailure) {
  if (!onFailure) {
    onFailure = function () {};
  }
  ...
  nvhttpHost.pollServer(function (ret) {
    ...
    $("#pairingDialogText").html(
      "Please enter the number " +
        randomNumber +
        " on the GFE dialog on the computer.  This dialog will be dismissed once complete"
    );
    ...
    nvhttpHost.pair(hostSessionKey).then(
      function () {
...
function hostChosen(host) {
  console.log("inside hostChosen : ", host);
  if (!host.online) {
    return;
  }
...
function connectToOneplayStreaming(
  ...
  _nvhttpHost.refreshServerInfoAtAddress(serverIp).then(
    function (success) {
      ...
      if (hosts[_nvhttpHost.serverUid] != null) {
        ...
        pairTo(_nvhttpHost, hostSessionKey, function () {
          // Host must be in the grid before starting background polling
    ...
    function (failure) {
      snackbarLog(
        "connectToOneplayStreaming failure Failed to connect to " +
          _nvhttpHost.hostname +
          "! Ensure that GameStream is enabled in GeForce Experience."
      );
    }.bind(this)
  );
...
/* function addHost() {
  ...
  $("#continueAddHost").on("click", function () {
    ...
    _nvhttpHost.refreshServerInfoAtAddress(inputHost).then(
      function (success) {
        modal.close();
        Navigation.pop();
...
function showApps(host) {
  ...
  host.getAppList().then(
    function (appList) {
      ...
      const desktopApp = appList.find((app) => app.title === "Desktop");
      console.log("desktopapp : ", desktopApp);
      if (desktopApp) {
        startGame(host, desktopApp.id);
      }
      // if game grid is populated, empty it
      const sortedAppList = sortTitles(appList, "ASC");
      console.log("applist : ", sortedAppList);
      /*
        ...
        if ($("#game-" + app.id).length === 0) {
          ...
          gameCard.addEventListener("keydown", (e) => {
            ...
            if (e.key == "ArrowLeft") {
              ...
              if (prev !== null) gameCard.previousSibling.focus();
              // TODO: Add a sound when limit reached
            }
            if (e.key == "ArrowRight") {
              let next = gameCard.nextSibling;
              if (next !== null) gameCard.nextSibling.focus();
              // TODO: Add a sound when limit reached
            }
          });
        ...
        host.getBoxArt(app.id).then(
          ...
          function (failedPromise) {
            console.log(
              "%c[index.js, showApps]",
              "color: green;",
              "Error! Failed to retrieve box art for app ID: " +
                app.id +
                ". Returned value was: " +
                failedPromise,
              "\n Host object:",
    ...
    function (failedAppList) {
      ...
      console.error(
        "%c[index.js, showApps]",
        "Failed to get applist from host: " + host.hostname,
        "\n Host object:",
        host,
        host.toString()
      );
...
function startGame(host, appID) {
  if (!host || !host.paired) {
    console.error(
      ...
      "color: green;",
      "Attempted to start a game, but `host` did not initialize properly. Host object: ",
      host
    );
    return;
  }
  ...
  if (gameStartedSuccessfully) {
    gameReconnectTryCount++;
    if (gameReconnectTryCount > 3) {
      goToReact();
      return;
    }
  }
  ...
  host.refreshServerInfo().then(function (ret) {
    host.getAppById(appID).then(function (appToStart) {
        ...
        host.getAppById(host.currentGame).then(
          function (currentApp) {
            ...
            document.getElementById("quitAppDialogText").innerHTML =
              currentApp.title +
              " is already running. Would you like to quit " +
              currentApp.title +
              "?";
          ...
          function (failedCurrentApp) {
            console.error(
              "[index.js, startGame]",
              "color: green;",
              "Failed to get the current running app from host! Returned error was:" +
                failedCurrentApp,
              "\n Host object:",
              host,
              host.toString()
      ...
      var streamWidth = urlParams.get("resolution").toString().split("x")[0]; //$("#selectResolution").data("value").split(":")[0];
          ...
          .then(
            function (launchResult) {
              ...
              sendMessage("startRequest", [
                ...
                framePacingEnabled,
                audioSyncEnabled,
                `rtsp://${host.address}:${host.rtspPort}`,
                host.controlPort,
                host.audioPort,
                host.videoPort,
              ]);
...
function stopGame(host, callbackFunction) {
  ...
  console.log("refreshServerInfo from stopGame");
  host.refreshServerInfo().then(
    function (ret) {
      host.getAppById(host.currentGame).then(
        function (runningApp) {
          if (!runningApp) {
          ...
          host.quitApp().then(
            function (ret2) {
              console.log("refreshServerInfo from quitApp ret2");
              host.refreshServerInfo().then(
                function (ret3) {
  ...
  let cb = function () {
    try {
      ...
      readRequest.onsuccess = function (event) {
        console.log(
          "Read data from the DB key: " + key + " value: " + readRequest.result
        );
        let value = null;
      ...
      transaction.onerror = function (e) {
        console.error(
          'Erorr reading data at key: "' + key + '" from IndexDB: ' + e
        );
        callCb(key, value, callbackFunction);
      };
    } catch (e) {
  ...
  let cb = function () {
    try {
      ...
      const put = transaction
        ...
        .put(JSON.stringify(data), key);

      transaction.oncomplete = function (e) {
        console.log(
          "Data at key: " + key + " stored as: " + JSON.stringify(data)
        );
        if (callbackFunction) {
          callbackFunction();
        }
...
function saveOptimize() {
  ...
  setTimeout(function () {
    var chosenOptimize = $("#optimizeGamesSwitch")
      .parent()
      .hasClass("is-checked");
    console.log(
      "%c[index.js, saveOptimize]",
      "color: green;",
      "Saving optimize state : " + chosenOptimize
    );
...
function saveFramePacing() {
  setTimeout(function () {
    const chosenFramePacing = $("#framePacingSwitch")
      .parent()
      .hasClass("is-checked");
    console.log(
      "%c[index.js, saveFramePacing]",
      "color: green;",
      "Saving framePacing state : " + chosenFramePacing
    );
...
function saveAudioSync() {
  setTimeout(function () {
    const chosenAudioSync = $("#audioSyncSwitch")
      .parent()
      .hasClass("is-checked");
    console.log(
      "%c[index.js, saveAudioSync]",
      "color: green;",
      "Saving audio sync state : " + chosenAudioSync
    );
...
function saveRemoteAudio() {
  ...
  // checking the new state
  setTimeout(function () {
    var remoteAudioState = $("#remoteAudioEnabledSwitch")
      .parent()
      .hasClass("is-checked");
    console.log(
      "%c[index.js, saveRemoteAudio]",
      "color: green;",
      "Saving remote audio state : " + remoteAudioState
    );
...
/* function loadUserDataCb() {
    ...
    if (previousValue.resolution != null) {
      $(".resolutionMenu li").each(function () {
        if ($(this).data("value") === previousValue.resolution) {
          $("#selectResolution")
            .text($(this).text())
            .data("value", previousValue.resolution);
        }
      });
    }
  });
    ...
    if (previousValue.frameRate != null) {
      $(".framerateMenu li").each(function () {
        if ($(this).data("value") === previousValue.frameRate) {
          $("#selectFramerate")
            .text($(this).text())
            .data("value", previousValue.frameRate);
        }
      });
    }
  });
  ...
  getData("bitrate", function (previousValue) {
    $("#bitrateSlider")[0].MaterialSlider.change(
      previousValue.bitrate != null ? previousValue.bitrate : "10"
    );
    updateBitrateField();
  });
} */
...
function createHTTPCert() {
  // we couldn't load a cert. Make one.
  console.warn(
    "%c[index.js, moduleDidLoad]",
    "color: green;",
    "Failed to load local cert. Generating new one"
  );
  sendMessage("makeCert", [])
    .then(
      function (cert) {
      ...
      function (failedCert) {
        console.error(
          "%c[index.js, moduleDidLoad]",
          "color: green;",
          "Failed to generate new cert! Returned error was: \n",
          failedCert
        );
      }
    )
    .then(function (ret) {
      sendMessage("httpInit", [
        pairingCert.cert,
        pairingCert.privateKey,
        myUniqueid,
      ]).then(
        function (ret) {
          restoreUiAfterNaClLoad();
          connectToOneplayStreaming(
            urlParams.get("server_ip"),
            urlParams.get("http_port"),
            urlParams.get("https_port"),
            urlParams.get("host_session_key"),
            urlParams.get("rtsp_port"),
            urlParams.get("control_port"),
            urlParams.get("audio_port"),
            urlParams.get("video_port")
          );
        },
        function (failedInit) {
          console.error(
            "%c[index.js, moduleDidLoad]",
            "color: green;",
            "Failed httpInit! Returned error was: ",
            failedInit
          );
        }
      );
  ...
  getData("cert", function (savedCert) {
    ...
    getData("uniqueid", function (savedUniqueid) {
      ...
      getData("hosts", function (previousValue) {
        ...
        for (var hostUID in hosts) {
          // programmatically add each new host.
          var revivedHost = new NvHTTP(
            hosts[hostUID].address,
            myUniqueid,
            hosts[hostUID].userEnteredAddress,
            hosts[hostUID].httpPort,
            hosts[hostUID].httpsPort,
            hosts[hostUID].rtspPort,
            hosts[hostUID].controlPort,
            hosts[hostUID].audioPort,
            hosts[hostUID].videoPort
          );
        ...
        console.log(
          "%c[index.js]",
          "color: green;",
          "Loaded previously connected hosts"
        );
      });
    });
  });
...
function onWindowLoad() {
  console.log(
    "%c[index.js]",
    "color: green;",
    "Moonlight's main window loaded"
  );
