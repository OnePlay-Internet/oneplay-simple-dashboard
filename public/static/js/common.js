...
var common = (function () {
  ...
  function browserSupportsNaCl(tool) {
    ...
    console.log(
      "%c[browserSupportsNaCl, common.js]",
      "color: gray;",
      "is NOT host toolchain for tool: " + tool
    );
  ...
  function createNaClModule(name, tool, path, width, height, attrs) {
    ...
    console.log(
      "%c[createNaClModule, common.js]",
      "color: gray;",
      "moduleEl: " + JSON.stringify(moduleEl)
    );
  ...
  function logMessage(message) {
    ...
    if (logMessageArray.length > kMaxLogMessageLength) logMessageArray.shift();
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (body.dataset) {
    ...
    if (loadFunction) {
      ...
      var tc =
        toolchains.indexOf(searchVars.tc) !== -1
          ? searchVars.tc
          : toolchains[0];
      ...
      else if (configs.indexOf("Release") !== -1) var config = "Release";
