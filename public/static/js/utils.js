...
NvHTTP.prototype = {
    ...
    return sendMessage("openUrl", [
      ...
      function (ret) {
        if (!this._parseServerInfo(ret)) {
          // if that fails
          console.log(
            "%c[utils.js, utils.js, refreshServerInfoAtAddress]",
            "color: gray;",
            "Failed to parse serverinfo from HTTPS, falling back to HTTP"
          );
          // try HTTP as a failover.  Useful to clients who aren't paired yet
          return sendMessage("openUrl", [
            "http://" + givenAddress + ":" + this.httpPort + "/serverinfo?" + this._buildUidStr(),
  ...
  selectServerAddress: function (onSuccess, onFailure) {
    ...
    this.refreshServerInfoAtAddress(this.address).then(
      function (successPrevAddr) {
        onSuccess(this.address);
      }.bind(this),
      function (successPrevAddr) {
        this.refreshServerInfoAtAddress(this.hostname + ".local").then(
          function (successLocal) {
            onSuccess(this.hostname + ".local");
          }.bind(this),
          function (failureLocal) {
            this.refreshServerInfoAtAddress(this.externalIP).then(
              function (successExternal) {
                onSuccess(this.externalIP);
              }.bind(this),
              function (failureExternal) {
                this.refreshServerInfoAtAddress(this.userEnteredAddress).then(
                  function (successUserEntered) {
                    onSuccess(this.userEnteredAddress);
                  }.bind(this),
                  function (failureUserEntered) {
                    console.warn(
                      "%c[utils.js, utils.js,  selectServerAddress]",
                      "color: gray;",
                      "Failed to contact host " + this.hostname,
                      this
                    );
                    onFailure();
                  }.bind(this)
  ...
  _parseServerInfo: function (xmlStr) {
    ...
    $root = $xml.find("root");
    console.log("server info status code : ", $root.attr("status_code"));
  ...
  pair: function (randomNumber) {
    console.log("refreshServerInfo from pair");
    return this.refreshServerInfo().then(
      function () {
        ...
        return sendMessage("pair", [this.serverMajorVersion.toString(), this.address, this.httpPort, randomNumber]).then(
          function (ppkstr) {
            ...
            return sendMessage("openUrl", [
              ...
              function (ret) {
                $xml = this._parseXML(ret);
                console.log("pairing final result  : ", ret);
