...
function pairTo(nvhttpHost, onSuccess, onFailure) {
  ...
  nvhttpHost.pollServer(function (ret) {
    ...
    if (nvhttpHost.currentGame != 0) {
      snackbarLog(
        nvhttpHost.hostname +
          " is currently in game. Quit the running app or restart the computer, then try again."
      );
      onFailure();
      return;
    }
    ...
    nvhttpHost.pair(randomNumber).then(
      function () {
...
function startGame(host, appID) {
  ...
  host.refreshServerInfo().then(function (ret) {
    host.getAppById(appID).then(function (appToStart) {
      ...
      var optimize = $("#optimizeGamesSwitch").parent().hasClass("is-checked")
        ? 1
        : 0;
      ...
      var mouse_lock_enabled = $("#mouseLockEnabledSwitch")
        .parent()
        .hasClass("is-checked")
        ? "1"
        : "0";
          ...
          function (launchResult) {
            ...
            if (status_code != 200) {
              ...
              if (status_code == 4294967295 && status_message == "Invalid") {
                ...
                status_code = 418;
                status_message =
                  "Missing audio capture device. Reinstall GeForce Experience.";
              }
...
function storeData(key, data, callbackFunction) {
  var obj = {};
  obj[key] = data;
  if (chrome.storage) chrome.storage.sync.set(obj, callbackFunction);
}
...
function saveMouseLock() {
  ...
  // checking the new state
  setTimeout(function () {
    var mouseLockState = $("#mouseLockEnabledSwitch")
      .parent()
      .hasClass("is-checked");
    console.log(
      "%c[index.js, saveMouseLock]",
      "color: green;",
      "Saving mouse lock state : " + mouseLockState
    );
  ...
  if (chrome.storage) {
    ...
    chrome.storage.sync.get("remoteAudio", function (previousValue) {
      if (previousValue.remoteAudio == null) {
        document
          .querySelector("#externalAudioBtn")
          .MaterialIconToggle.uncheck();
    ...
    chrome.storage.sync.get("optimize", function (previousValue) {
      if (previousValue.optimize == null) {
        ...
        document
          .querySelector("#optimizeGamesBtn")
          .MaterialIconToggle.uncheck();
      } else {
        document.querySelector("#optimizeGamesBtn").MaterialIconToggle.check();
      }
