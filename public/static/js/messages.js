...
function handleMessage(msg) {
  if (msg.data.callbackId && callbacks[msg.data.callbackId]) {
    // if it's a callback, treat it as such
    callbacks[msg.data.callbackId][msg.data.type](msg.data.ret);
    delete callbacks[msg.data.callbackId];
  } else {
    // else, it's just info, or an event
    console.log(
      "%c[messages.js, handleMessage]",
      "color:gray;",
      "Message data: ",
      msg.data
    );
    if (msg.data.indexOf("streamTerminated: ") === 0) {
      ...
      navigator
        .getGamepads()
        [gamepadIdx]?.vibrationActuator?.playEffect("dual-rumble", {
          startDelay: 0,
